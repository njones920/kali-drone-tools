...
[ -f "$MENU_FILE" ] || die "Kali application menu not found: $MENU_FILE"

if ! grep -Fq "$MENU_MARKER" "$MENU_FILE"; then
    patch --dry-run --forward "$MENU_FILE" \
        "$ROOT_DIR/menu/kali-applications.menu.patch" >/dev/null ||
        die "the installed Kali menu is not compatible; no changes were made"
fi

shopt -s nullglob
metapackages=("$ROOT_DIR"/packages/kali-tools-drone_*.deb)
shopt -u nullglob

[ "${#metapackages[@]}" -eq 1 ] ||
    die "expected exactly one kali-tools-drone package in packages/"
...
info "Installing the drone metapackage"
apt-get install -y "${metapackages[0]}"
...
if grep -Fq "$MENU_MARKER" "$MENU_FILE"; then
    ...
    patch --forward "$MENU_FILE" \
        ...
        die "the installed Kali menu is not compatible; original menu was preserved"
fi
